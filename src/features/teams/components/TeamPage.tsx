...
import { Plus, Crown, UserPlus, Trash2, Pencil } from "lucide-react";
...
export function TeamPage() {
  ...
  const { data: orgMembers = [] } = trpc.teams.organizationMembers.useQuery(
    undefined,
    { enabled: isAdmin },
  );
  ...
  return (
    <div className="crm-content">
      ...
      {isAdmin && (
        <AdminTeamsPanel teams={allTeams} members={orgMembers} />
      )}
...
function AdminTeamsPanel({ teams, members }: { teams: any[]; members: any[] }) {
          ...
          <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
            ...
            <details style={{ position: "relative" }}>
              <summary
                ...
                style={{ listStyle: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}
              >
                ...
                {members
                  ...
                  .map((m: any) => (
                    <button
                      ...
                      onClick={() => setMembership.mutate({ userId: m.id, teamId: t.id })}
                    >
                      <div className={`crm-avatar xs ${avatarClass(m.name)}`}>
                        {initials(m.name || m.email)}
                      </div>
                      <span>
                        {m.name || m.email}
                        {m.team && <span style={{ color: "var(--crm-fg-faint)" }}> · {m.team.name}</span>}
                      </span>
                    </button>
                  ))}
              </div>
