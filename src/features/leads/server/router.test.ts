...
describe("leadsRouter", () => {
  ...
  describe("updateCallOutcome", () => {
    ...
    it("creates a follow-up task due tomorrow when outcome is NO_ANSWER", async () => {
      prisma.lead.findFirst.mockResolvedValue({
        id: "lead-1",
        organizationId: "org-1",
        callOutcome: "ANSWERED",
      ...
      const dueDate: Date = prisma.task.create.mock.calls[0][0].data.dueDate;
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      expect(dueDate.getDate()).toBe(tomorrow.getDate());
    });

    it("falls back to company name in follow-up task title when no first/last name", async () => {
      prisma.lead.findFirst.mockResolvedValue({
        id: "lead-1",
        organizationId: "org-1",
        callOutcome: "NOT_CONTACTED",
    ...
    it("does not create a follow-up task for non-NO_ANSWER outcomes", async () => {
      prisma.lead.findFirst.mockResolvedValue({
        id: "lead-1",
        organizationId: "org-1",
        callOutcome: "NOT_CONTACTED",
      ...
      prisma.lead.update.mockResolvedValue({ id: "lead-1", callOutcome: "ANSWERED" });
